...
async def startup_event():
    """服务器启动时执行登录并显示信息"""
    try:
        from ..warp.login import client_login
        logger.info("正在执行客户端登录...")
        success = await client_login()
        if not success:
            logger.warning("客户端登录失败，但服务器将继续运行")
    except Exception as e:
        logger.warning(f"客户端登录异常: {e}")
        ...
        async def _agen():
            ...
            async with httpx.AsyncClient(http2=True, timeout=httpx.Timeout(60.0), verify=verify_opt, trust_env=True) as client:
                ...
                for attempt in range(2):
                    ...
                    # Build request to remove user-agent (SSE endpoint)
                    request_obj = client.build_request("POST", warp_url, headers=headers, content=protobuf_bytes)
                    if 'user-agent' in request_obj.headers:
                        del request_obj.headers['user-agent']
                        logger.debug("Removed user-agent header (SSE endpoint)")
