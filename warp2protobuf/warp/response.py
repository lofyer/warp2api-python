                ...
                if action.HasField("append_to_message_content"):
                    ...
                    for j, msg in enumerate(action.add_messages_to_task.messages):
                        ...
                        if msg.HasField("tool_call"):
                            ...
                            tool_name = "unknown"
                            tool_args = "{}"
                            # 直接访问 tool_call_id 字段
                            tool_call_id = tool_call.tool_call_id if tool_call.tool_call_id else f"call_{i}_{j}"
                            logger.debug(f"Extracted tool_call_id from add_messages_to_task: {tool_call_id}")
                            for field, value in tool_call.ListFields():
                                if field.name == 'tool_call_id':
                                    continue
                                tool_name = field.name
                                
                                # 跳过 server 类型 - 这是 Warp 内部事件，不是真正的工具调用
                                if tool_name == 'server':
                                    logger.debug(f"Skipping internal 'server' event")
                                    continue
                                
                                if hasattr(value, 'ListFields'):
                                    tool_fields_dict = {}
                                    # 特殊处理 call_mcp_tool，提取内部的 name 作为实际工具名
                                    if tool_name == 'call_mcp_tool':
                                        ...
                                        for tool_field, tool_value in value.ListFields():
                                            if tool_field.name == 'name':
                                                ...
                                                tool_fields_dict['name'] = tool_value
                                            elif tool_field.name == 'args':
                                                # args 是 Struct 类型，需要转换为 dict
                                                tool_fields_dict['args'] = MessageToDict(tool_value)
                                            else:
                                                tool_fields_dict[tool_field.name] = str(tool_value)
                                        # 使用实际的工具名（如 LS, Read, Grep）而不是 call_mcp_tool
                                        if actual_tool_name:
                                            tool_name = actual_tool_name
                                        tool_args = json.dumps(tool_fields_dict.get('args', {}))
                                    else:
                                        # 其他工具的正常处理
                                        for tool_field, tool_value in value.ListFields():
                            ...
                            # 只有有效的工具调用才添加到结果中
                            if tool_name != 'unknown' and tool_name != 'server':
                                openai_tool_call = {
                                    "id": tool_call_id,
                                    "type": "function",
            ...
            for i, action in enumerate(response.client_actions.actions):
                if action.HasField("append_to_message_content"):
                    ...
                    if message.HasField("tool_call"):
                        ...
                        # 提取工具调用信息 - 直接访问 tool_call_id 字段
                        tool_call_id = tool_call.tool_call_id if tool_call.tool_call_id else f"call_{i}"
                        logger.debug(f"Extracted tool_call_id from append_to_message_content: {tool_call_id}")
                        tool_name = "unknown"
                        tool_args = "{}"
                        
                        # tool_call 是一个 oneof，需要遍历找到实际的工具
                        for field, value in tool_call.ListFields():
                            if field.name == 'tool_call_id':
                                continue
                            # field.name 就是工具名称（如 call_mcp_tool, run_shell_command 等）
                            tool_name = field.name
                            # value 是工具的参数消息
                            if hasattr(value, 'ListFields'):
                                tool_fields_dict = {}
                                # 特殊处理 call_mcp_tool，它的 args 是 google.protobuf.Struct
                                if tool_name == 'call_mcp_tool':
                                    from google.protobuf.json_format import MessageToDict
                                    import json
                                    # 提取 name 和 args
                    ...
                    for j, msg in enumerate(action.add_messages_to_task.messages):
                        ...
                        if msg.HasField("tool_call"):
                            ...
                            # 直接访问 tool_call_id 字段
                            tool_call_id = tool_call.tool_call_id if tool_call.tool_call_id else f"call_{i}_{j}"
                            logger.debug(f"Extracted tool_call_id from add_messages_to_task (streaming): {tool_call_id}")
                            tool_name = "unknown"
                            tool_args = "{}"
                            for field, value in tool_call.ListFields():
                                if field.name == 'tool_call_id':
